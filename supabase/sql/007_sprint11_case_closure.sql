-- Sprint 11 migration: finalize case closure permissions + audit action

create or replace function is_case_reviewer(p_case_id uuid, p_org_id uuid)
returns boolean
language sql
stable
as $$
  select exists (
    ...
    where offboarding_cases.id = p_case_id
      and offboarding_cases.org_id = p_org_id
...
begin
  ...
  select status, reviewer_user_id is not null
    into current_status, reviewer_assigned
  from offboarding_cases
  where id = p_case_id
    and org_id = p_org_id
  for update;
